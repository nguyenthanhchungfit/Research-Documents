...
* Exercising Our Privileges
    ...
    But we still have a problem. With the current permissions, files and directories created
        ...
        Actually there are two problems.
            First, the default umask on this system is 0022 which prevents group members 
                from writing files belonging to other members of the group.
                This would not be a problem if the shared directory only contained files, 
                but since this directory will store music, and music is usually organized 
                in a hierarchy of artists and albums, members of the group will need 
                the ability to create files and directories inside directories created by other members.
            ...
            Second, each file and directory created by one member will be set to the primary group of
                the user rather than the group music. This can be fixed by setting the setgid bit on the directory
...
* Changing Your Password
    ...
    To change your password, just enter the passwd command.
        ...
        -> The passwd command will try to enforce use of “strong” passwords. This means it will
            refuse to accept passwords that are too short, too similar to previous passwords, 
            are dictionary words, or are too easily guessed
